...
function chart(principal, interest, monthly, payments) {
  ...
  //Se chamado sem argumentos ou se esse navegador não suporta elementos gráficos em um elemento <canvas>, basta retornar agora.
  ...
  g.moveTo(paymentToX(0), amountToY(0)); //Começa no canto inferior esuerdo
  ...
  g.moveTo(paymentToX(0), amountToY(0)); //começando no canto superior esquerdo

  for (var p = 1; p <= payments; p++) {
    //Para cada pagamento, descobrequanto é o juro
  ...
  g.textAlign = "rigth"; //Alinha o texto a direita
  g.textBasilane = "middle"; //Centraliza verticalmente
  var ticks = [monthly * payments, principal]; //Os dois pontos que marcamos
  var rigthEdge = paymentToX(payments); //Coordenada x do eixo y
  for (var i = 0; i < ticks.length; i++) {
    //Para cada um dos dois pontos
    var y = amountToY(ticks[i]); //Calcula a posição Y da marca
    g.fillRect(rigthEdge - 3, y - 0.5, 3, 1); //Desenha a marcação
    g.fillText(String(ticks[1].toFixed(0)), rigthEdge - 5, y); //E a rotula
  }
}